...
from utils import get_metadata, get_image_url
...
class VolumioSocket:
    def __init__(self,session, port=5000):
        ...
        print 'starting metadata socket on %s port %s' % self._address
        self.sendmsg('Metadata soket initialted!')

    def onLoggedIn(self):
        self.sendmsg("kSpConnectionNotifyLoggedIn")

    def onLoggedOut(self):
        self.sendmsg("kSpConnectionNotifyLoggedOut")

    def onSessionActivate(self):
        self.sendmsg("kSpPlaybackNotifyBecameActive")
        self.sendMeta()

    def onSessionInactive(self):
    ...
    def onDeviceInactive(self):
        self.sendmsg("kSpDeviveInactive", self._session.is_active())
    ...
    def sendmsg(self,msg, Sactive=True):
        if Sactive:
            self.socket.sendto(msg, self._address)
...
def connection_notify(self, type):
    if type == lib.kSpConnectionNotifyLoggedIn:
        ...
        VolSoc.sendmsg("kSpConnectionNotifyTemporaryError")
    else:
